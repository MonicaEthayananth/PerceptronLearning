...
class ClNeuralNetwork:
    ...
    def adjust_weights(self, input_samples, error):
       ...
       for layer_index, layer in enumerate(self.layers):
           layer.weights[:,:-1] += np.dot(self.learning_rate,np.dot(error,np.transpose(input_samples)))
           x = np.reshape(layer.weights[:,-1],(-1,1))
           x += np.dot(self.learning_rate,np.dot(error,(np.ones(shape=(input_samples.shape[1],1)))))
           layer.weights[:,-1] = x.flatten()
